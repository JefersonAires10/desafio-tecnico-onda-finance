...
import { TransferFormData } from '@/features/transfers/schemas/transferSchema'
...
export function performTransfer(data: Omit<TransferFormData, 'amount'> & { amount: number }, currentBalance: number) {
  return new Promise<{ newBalance: number }>((resolve, reject) => {
    setTimeout(() => {
      if (data.amount > currentBalance) {
        reject(new Error('Saldo insuficiente para realizar a transferência.'))
      } else {
